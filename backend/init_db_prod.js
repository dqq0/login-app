...
const PROD_DB = 'death_cloud_prod';
...
async function run() {
  ...
  for (let q of mainQueries) {
    await pool.query(q);
    console.log('  ✅', q.trim().replace(/\s+/g, ' ').substring(0, 70));
  }
  ...
  for (let game of games) {
    ...
    if (parseInt(check.rows[0].count) === 0) {
      console.log(`  🌱 Sembrando leaderboard...`);
      for (let entry of defaultStats[s]) {
        const uid = ++fakeId;
        await pool.query(
          `INSERT INTO usuarios (id, nombre_usuario, nickname, email, clave_encriptada) VALUES ($1, $2, $2, $3, 'mock_prod_hash') ON CONFLICT DO NOTHING`,
      ...
      console.log(`  ℹ️  Stats ya poblados. Saltando seed.`);
    }
  }
